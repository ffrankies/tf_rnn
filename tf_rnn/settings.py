...
class Settings(object, metaclass=Singleton):
    ...
    def __init__(self, dataset_only: bool = False) -> None:
    ...
    def __str__(self) -> str:
        ...
        Return:
            str: A string representation of the Namespaces comprising this Settings object.
        """
    ...
    def _get_config_dicts(self, dataset_only: bool) -> Tuple:
        ...
        Return:
            Tuple: Separate dictionaries for each configuration category.
        """
    ...
    def _parse_config_yml(self, config_file: str) -> Tuple:
        """Parses the YAML config file into multiple dictionaries.

        Params:
            config_file (str): The path to the config file from which to load settings

        Return:
            Dict[str, Any]: The general settings
            Dict[str, Any]: The logging settings
            Dict[str, Any]: The RNN/model settings
            Dict[str, Any]: The training settings
            Dict[str, Any]: The dataset settings
        """
    ...
    def _parse_config_args(self, args: argparse.Namespace) -> Tuple:
        ...
        general = self._get_arg_subset(args, vars(self.general).keys())
        logging = self._get_arg_subset(args, vars(self.logging).keys())
        rnn = self._get_arg_subset(args, vars(self.rnn).keys())
        train = self._get_arg_subset(args, vars(self.train).keys())
        data = self._get_arg_subset(args, vars(self.data).keys())
        return general, logging, rnn, train, data
    # End of _parse_config_args()

    def _get_arg_subset(self, args: argparse.Namespace, arg_keys: Iterable) -> Dict[str, Any]:
        ...
        Return:
            Dict[str, Any]: The general settings passed into the program
        """
        arg_dict = vars(args)
        new_dict = dict()
