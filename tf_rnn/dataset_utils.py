...
def save_dataset(logger: Logger, args: settings.DatasetSettings):
    """Saves the created dataset to a specified file.

    Params:
        logger (logging.Logger): The logger used in this run of the script
        args (settings.DatasetSettings): The dataset creation settings
    """
...
def create_dataset(logger: Logger, args: settings.DatasetSettings):
    ...
    Return:
        string: The type of dataset ('text' or 'number')
        string: The level of tokenization ('sentences', 'paragraphs', or 'stories')
        list: The vocabulary used in the dataset
        list: Converts indexes to tokens
        dict: Converts tokens to indexes
...
def create_text_dataset(logger: Logger, args: settings.DatasetSettings, dataset: tuple) -> tuple:
    ...
    Return:
        string: The type of dataset. 'text', in this case
        string: The level of tokenization ('sentences', 'paragraphs', or 'stories')
        List: The vocabulary used in the dataset
        List: Converts indexes to tokens
        Dict: Converts tokens to indexes
...
def create_numeric_dataset(logger: Logger, args: settings.DatasetSettings, dataset: tuple) -> tuple:
...
def tokenize_data(logger: Logger, args: settings.DatasetSettings) -> list:
    """Creates a dataset using tokenized data.

    Params:
        logger (logging.Logger): The logger used in this run of the script
        args (settings.DatasetSettings): The dataset creation args
...
def tokenize_sentences(logger: Logger, args: settings.DatasetSettings) -> list:
...
def tokenize_paragraphs(logger: Logger, args: settings.DatasetSettings) -> list:
...
def tokenize_stories(logger: Logger, args: settings.DatasetSettings) -> list:
...
def read_csv(logger: Logger, args: settings.DatasetSettings) -> list:
...
def normalize_examples(logger: Logger, args: settings.DatasetSettings, examples: list) -> list:
...
def preprocess_data(logger: Logger, data_array: list) -> list:
    ...
    Return:
        list: The list of preprocessed strings.
    """
...
def low_level_tokenize(logger: Logger, args: settings.DatasetSettings, examples: list):
...
def create_vocabulary(logger: Logger, args: settings.DatasetSettings, data: list):
...
def create_training_data(logger: Logger, data: np.ndarray, translators: Translators) -> tuple:
    """Creates the inputs and labels for training.

    Params:
        logger (logging.Logger): The logger used in this run of the script
        data (np.ndarray): The data to break into inputs and labels
        translators (Translators): The translators for translating data

    Return:
        list: The training inputs
        list: The training labels (outputs)
    """
    ...
    print('Shape of data: {} dtype: {}'.format(np.shape(data), data.dtype))
    translated_data = translators.translate_all(data)
    print('Shape of translated data: {} Dtype: {}'.format(np.shape(translated_data), translated_data.dtype))
    x_train = [sequence[:-1] for sequence in translated_data]
    output_names = list()
    for index in translators.output_indexes:
        output_name = translated_data.dtype.names[index]
        output_names.append(output_name)
    outputs_only = translated_data[output_names]
    y_train = [sequence[1:] for sequence in outputs_only]
...
def tokens_to_indexes(logger: Logger, data: list, token_to_index: dict) -> list:
    ...
    Return:
        list: The data with the tokens replaced with their indexes in the vocabulary
    """
...
def load_dataset(logger: Logger, dataset: str) -> tuple:
    ...
    with open(path, "rb") as dataset_file:
        ...
        logger.info("The number of output features is: {:d}".format(translators.num_output_translators))
        logger.info("The maximum sequence length is: {:d}".format(max_sequence_length))
